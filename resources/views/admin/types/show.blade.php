...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 px-4">
        <h1 class="h3 fw-bold mb-0 font-display">{{ __("Category Details") }}</h1>
    ...
    <div class="card card-accent-left">
        <div class="card-body p-4">
            ...
            @if ($type->projects->isNotEmpty())
                <div class="mt-4">
                    <h5 class="text-secondary mb-3">{{ __("Projects in this category") }}</h5>
            ...
            @else
                <p class="text-secondary mt-3"><i class="bi bi-info-circle me-1"></i> {{ __("No projects in this category.") }}</p>
            @endif
            ...
            <div class="d-flex justify-content-between align-items-center">
                <form action="{{ route('admin.types.destroy', $type->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-outline-danger shadow-sm" onclick="return confirm('{{ __("Are you sure you want to delete this category?") }}')">
